execute @s[scores={mdmtoggle=1}] ~~~ execute @s[scores={stafftag=1}] ~~~ scoreboard players set mdmtoggledummy goldmd 0
...
execute @s[scores={mdmtoggle=1}] ~~~ execute @s[scores={stafftag=1}] ~~~ tellraw @a[scores={stafftag=1}] {"rawtext":[{"text":"§¶§cAtomic §¶§b→ §6Gold Alerts §bhas been toggled §cOFF §bby §d"},{"selector":"@s"}]}
