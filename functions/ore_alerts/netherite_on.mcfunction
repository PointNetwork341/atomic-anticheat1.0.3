execute @s[scores={mdmtoggle=1}] ~~~ execute @s[scores={stafftag=1}] ~~~ scoreboard players set mdmtoggledummy scrapmd 1
...
execute @s[scores={mdmtoggle=1}] ~~~ execute @s[scores={stafftag=1}] ~~~ tellraw @a[scores={stafftag=1}] {"rawtext":[{"text":"§¶§cAtomic §¶§b→ §6Iron Alerts §bhas been toggled §2ON §bby §d"},{"selector":"@s"}]}
